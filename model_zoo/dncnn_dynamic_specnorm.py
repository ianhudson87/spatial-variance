...
class DnCNNDynamicSpecnormMoreOutputLayers(nn.Module):
    def __init__(self, channels, num_of_layers=13, verbose=False):
        ...
        self.dncnn_out2 = spectral_norm(nn.Conv2d(in_channels=features, out_channels=1, kernel_size=kernel_size, padding=padding, bias=False))


        feat_kernel_layers = []
        feat_kernel_layers.append(nn.Conv2d(cnn_out_features, 5**2, 3, 1, 1))
        for _ in range(5):
            feat_kernel_layers.append(nn.Conv2d(5**2, 5**2, 3, 1, 1))
            layers.append(nn.BatchNorm2d(5**2))
            layers.append(nn.ReLU(inplace=True))
...
class DnCNNDynamicSpecnormMoreDynamicLayers(nn.Module):
    def __init__(self, channels, num_of_layers=13, verbose=False):
        ...
        for _ in range(num_dynamic_layers):
            dynamic_layers.append(nn.Sequential(
                nn.Conv2d(1, 16, 3, 1, 1),
                nn.Conv2d(16, 32, 3, 1, 1),
                nn.Conv2d(32, 5**2, 3, 1, 1),
            ))
        
        self.dynamic_layers = nn.ModuleList(dynamic_layers)
